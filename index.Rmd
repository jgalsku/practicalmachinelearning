...
```{r}
# str(pml_training)
kable(table(pml_training$classe), "html")
```
&nbsp;
...
The Random Forest model was then used to predict the validation data classes and evaluate the accuracy of the predictions by comparing them to the known classifications. The model's accuracy was equal to `r acc_rf`, and the confusion matrix table is displayed below:  
```{r}
pml_pred_rf <- predict(pml_model_rf, newdata = validating)
acc_rf <- round(confusionMatrix(pml_pred_rf, factor(validating$classe))$overall[1],4) #get accuracy
...
The GBM model was then used to predict the validation data classes and evaluate the accuracy of the predictions by comparing them to the known classifications. The model's accuracy was equal to `r acc_gbm`, and the confusion matrix table is displayed below:  
```{r}
pml_pred_gbm <- predict(pml_model_gbm, newdata = validating)
acc_gbm <- round(confusionMatrix(pml_pred_gbm, factor(validating$classe))$overall[1],4)
...
pml_pred_testing
kable(table(pml_pred_testing), "html")
```
